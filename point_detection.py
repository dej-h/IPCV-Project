...
def order_points(points):
    ...
    # Split points into top and bottom based on sorted order
    top_points = sorted_points[:2]
    bottom_points = sorted_points[2:]

    # Sort the top and bottom points by x-coordinate to get left-right order
    ...
    point1 = top_points[1]  # top-right
    point2 = top_points[0]  # top-left
    selected_point1 = bottom_points[1]  # bottom-right
    selected_point2 = bottom_points[0]  # bottom-left
...
def video_player(frame_array):
    ...
    Args:
        frame_array (list of numpy arrays): Array of frames to display.
    ...
    while True:
        ...
        elif key == ord('f') and frame_index < len(frame_array) - 1:  # Forward
            frame_index += 1
        elif key == ord('b') and frame_index > 0:  # Backward
            frame_index -= 1

        if is_playing and frame_index < len(frame_array) - 1:
